...
def standard_pipeline_group():
    return GoCdConfigurator(
        config("config-with-typical-pipeline")
...
def more_options_pipeline():
    return (
        GoCdConfigurator(config("config-with-more-options-pipeline"))
        .ensure_pipeline_group("P.Group")
...
def more_options_pipeline_with_artifacts_type():
    return (
        GoCdConfigurator(
            config("config-with-more-options-pipeline-including-artifacts-type")
        )
        .ensure_pipeline_group("P.Group")
        .find_pipeline("more-options")
    )
...
def more_options_pipeline_with_external_artifacts():
    return (
        GoCdConfigurator(
            config_18_3_0("config-with-more-options-pipeline-including-artifacts-type")
        )
        .ensure_pipeline_group("P.Group")
        .find_pipeline("more-options")
    )
...
def empty_pipeline():
    return (
        GoCdConfigurator(empty_config())
        .ensure_pipeline_group("pg")
        .ensure_pipeline("pl")
        .set_git_url("gurl")
    )
...
class TestJobs(unittest.TestCase):
    ...
    def test_job_has_nice_tostring(self):
        ...
        self.assertEqual(
            "Job('compile', [ExecTask(['make', 'options', 'source code'])])", str(job)
        )
    ...
    def test_jobs_have_artifacts_with_type(self):
        job = (
            more_options_pipeline_with_artifacts_type()
            .ensure_stage("earlyStage")
            .ensure_job("earlyWorm")
        )
        artifacts = job.artifacts
        self.assertEqual(
            {
                ...
                Artifact.get_build_artifact("scripts/*", "files"),
                Artifact.get_test_artifact("from", "to"),
                Artifact.get_external_artifact(
                    "docker-image",
                    "docker-registry",
                    {
    ...
    def test_job_that_has_no_artifacts_has_no_artifacts_element_to_reduce_thrash(self):
        ...
        job = (
            go_cd_configurator.ensure_pipeline_group("g")
            .ensure_pipeline("p")
            .ensure_stage("s")
            .ensure_job("j")
        )
    ...
    def test_can_add_test_artifacts_to_job(self):
        job = more_options_pipeline().ensure_stage("earlyStage").ensure_job("earlyWorm")
        job_with_artifacts = job.ensure_artifacts(
            {Artifact.get_test_artifact("a1"), Artifact.get_test_artifact("a2")}
        )
        self.assertEqual(job, job_with_artifacts)
        artifacts = job.artifacts
        self.assertEqual(5, len(artifacts))
    ...
    def test_can_add_external_artifacts_to_job(self):
        ...
        job = (
            more_options_pipeline_with_external_artifacts()
            .ensure_stage("earlyStage")
            .ensure_job("earlyWorm")
        )
        job_with_artifacts = job.ensure_artifacts(
    ...
    def test_can_add_external_artifact_fetch_task_to_job(self):
        ...
        job_with_fetch_external_artifact_task = job.add_task(
            FetchArtifactTask(
                ...
                "build",
                id="docker-image",
                config={"EnvironmentVariablePrefix": None, "SkipImagePulling": "true"},
        ...
        job.ensure_artifacts(
            {
                ...
                Artifact.get_build_artifact(
                    "target/universal/myapp*.zip", "somewhereElse"
                ),
    ...
    def test_can_ensure_exec_task(self):
        ...
        self.assertEqual(
            ["make", "options", "source code"], (job.tasks[0]).command_and_args
        )
    ...
    def test_can_add_fetch_artifact_task_to_job(self):
        stages = typical_pipeline().stages
        job = stages[0].jobs[0]
        added_task = job.add_task(
            FetchArtifactTask("p", "s", "j", FetchArtifactDir("d"), runif="any")
        )
        self.assertEqual(2, len(job.tasks))
        task = job.tasks[1]
    ...
    def test_can_ensure_fetch_artifact_tasks(self):
        ...
        job.ensure_task(
            FetchArtifactTask(
                ...
                "middleStage",
                "middleJob",
                FetchArtifactFile("someFile"),
            )
        )
        first_added_task = job.ensure_task(
            FetchArtifactTask("p", "s", "j", FetchArtifactDir("dir"))
        )
        ...
        job.ensure_task(
            FetchArtifactTask("p", "s", "j", FetchArtifactDir("dir"), dest="somedest")
        )
        self.assertEqual("somedest", (job.tasks[6]).dest)

        job.ensure_task(
            FetchArtifactTask("p", "s", "j", FetchArtifactDir("dir"), runif="failed")
        )
    ...
    def test_can_have_encrypted_environment_variables(self):
        pipeline = (
            GoCdConfigurator(config("config-with-encrypted-variable"))
            .ensure_pipeline_group("defaultGroup")
            .find_pipeline("example")
        )
        job = pipeline.ensure_stage("defaultStage").ensure_job("defaultJob")
        self.assertEqual(
            {"MY_JOB_PASSWORD": "yq5qqPrrD9/j=="}, job.encrypted_environment_variables
        )
    ...
    def test_can_add_unencrypted_secure_environment_variables_to_stage(self):
        ...
        self.assertEqual(
            {"new": "one", "again": "two"}, job.unencrypted_secure_environment_variables
        )
    ...
    def test_environment_variables_get_added_in_sorted_order_to_reduce_config_thrash(
        self,
    ):
        go_cd_configurator = GoCdConfigurator(empty_config())
        ...
        self.assertEqual(["ant", "badger", "zebra"], names)
    ...
    def test_can_addTab(self):
        ...
        self.assertEqual(j, job)
        self.assertEqual(
            [Tab("Time_Taken", "artifacts/test-run-times.html"), Tab("n", "p")],
            job.tabs,
        )
...
class TestTasks(unittest.TestCase):
    ...
    def test_renders_fetch_artifact_task_with_dest_origin(self):
        ...
        fetch_artifact_task = FetchArtifactTask(
            "p", "s", "j", FetchArtifactDir("d"), dest="dest_path", origin="gocd"
        )
...
class TestStages(unittest.TestCase):
    ...
    def test_can_have_encrypted_environment_variables(self):
        ...
        self.assertEqual(
            {"MY_STAGE_PASSWORD": "yq5qqPrrD9/s=="},
            stage.encrypted_environment_variables,
        )
    ...
    def test_can_set_environment_variables(self):
        ...
        self.assertEqual(s, stage)
        self.assertEqual(
            {"BASE_URL": "http://myurl", "new": "one"}, stage.environment_variables
        )

    def test_can_add_unencrypted_secure_environment_variables_to_stage(self):
        stage = typical_pipeline().ensure_stage("deploy")
        stage.ensure_unencrypted_secure_environment_variables(
            {"new": "one", "again": "two"}
        )
        self.assertEqual(
            {"new": "one", "again": "two"},
            stage.unencrypted_secure_environment_variables,
        )
...
class TestConfigRepo(unittest.TestCase):
    ...
    def test_ensure_replacement_of_config_repos(self):
        self.configurator.ensure_config_repos().ensure_config_repo(
            "git://url", "yaml.config.plugin"
        )
        self.assertEqual(len(self.configurator.config_repos.config_repo), 1)

        self.configurator.ensure_replacement_of_config_repos().ensure_config_repo(
            "git://otherurl", "yaml.config.plugin"
        )
        self.assertEqual(len(self.configurator.config_repos.config_repo), 1)
    ...
    def test_can_ensure_config_repo_with_git_url_and_plugin(self):
        ...
        self.assertEqual(
            self.configurator.config_repos.config_repo[0].plugin, "yaml.config.plugin"
        )
    ...
    def test_can_ensure_config_repo_with_git_url_and_branch(self):
        self.configurator.ensure_config_repos().ensure_config_repo(
            "git://url", "yaml.config.plugin", branch="release"
        )
        ...
        self.assertEqual(
            self.configurator.config_repos.config_repo[0].branch, "release"
        )
    ...
    def test_can_ensure_json_config_repo_with_git_url(self):
        ...
        self.assertEqual(self.configurator.config_repos.config_repo[0].url, "git://url")
        self.assertEqual(
            self.configurator.config_repos.config_repo[0].plugin, "json.config.plugin"
        )

    def test_can_ensure_repo_for_different_cvs(self):
        self.configurator.ensure_config_repos().ensure_config_repo(
            "svn://url", "json.config.plugin", cvs="svn"
        )
    ...
    def test_can_ensure_config_repo_with_configuration(self):
        self.configurator.ensure_config_repos().ensure_config_repo(
            ...
            "yml.config.plugin",
            cvs="yml",
            configuration={"file_pattern": "*.gocd.yml"},
        )

        self.assertEqual(
            self.configurator.config_repos.config_repo[0].configuration,
            {"file_pattern": "*.gocd.yml"},
        )

    def test_can_ensure_replacement_of_config_repo(self):
        self.configurator.ensure_config_repos().ensure_config_repo(
            "git://url", "yml.config.plugin"
        )

        self.configurator.ensure_config_repos().ensure_replacement_of_config_repo(
            "git://url", "json.config.plugin"
        )
    ...
    def test_changes_attrs_for_new_server_versions(self):
        configurator = GoCdConfigurator(
            FakeHostRestClient(empty_config_xml, version="17.9.0")
        )
        configurator.ensure_config_repos().ensure_config_repo(
            "git://url", "yml.config.plugin", repo_id="myRepo"
        )
        self.assertEqual(
            configurator.config_repos.config_repo[0].element.get("pluginId"),
            "yml.config.plugin",
        )
        self.assertEqual(
            configurator.config_repos.config_repo[0].plugin, "yml.config.plugin"
        )
    ...
    def test_handles_unspecified_id_for_migration(self):
        configurator = GoCdConfigurator(
            FakeHostRestClient(empty_config_xml, version="17.8.0")
        )
        configurator.ensure_config_repos().ensure_config_repo(
            "git://url", "yml.config.plugin"
        )
...
class TestPipeline(unittest.TestCase):
    ...
    def test_git_material_can_ignore_sources(self):
        pipeline = (
            GoCdConfigurator(config("config-with-source-exclusions"))
            .ensure_pipeline_group("P.Group")
            .find_pipeline("with-exclusions")
        )
        self.assertEqual(
            {"excluded-folder", "another-excluded-folder"},
            pipeline.git_material.ignore_patterns,
        )
    ...
    def test_can_set_pipeline_git_url_with_options(self):
        ...
        self.assertEqual(
            {"ignoreMe", "ignoreThisToo"}, pipeline.git_material.ignore_patterns
        )
    ...
    def test_can_add_git_material(self):
        ...
        p = pipeline.ensure_material(
            GitMaterial("git@bitbucket.org:springersbm/changed.git")
        )
        self.assertEqual(p, pipeline)
        self.assertEqual("git@bitbucket.org:springersbm/changed.git", pipeline.git_url)
    ...
    def test_can_ensure_git_material(self):
        ...
        pipeline.ensure_material(
            GitMaterial("git@bitbucket.org:springersbm/gomatic.git")
        )
        self.assertEqual("git@bitbucket.org:springersbm/gomatic.git", pipeline.git_url)
        self.assertEqual(
            [GitMaterial("git@bitbucket.org:springersbm/gomatic.git")],
    ...
    def test_pipelines_can_have_pipeline_materials(self):
        ...
        self.assertEqual(
            GitMaterial(
                "git@bitbucket.org:springersbm/gomatic.git",
                branch="a-branch",
    ...
    def test_can_add_pipeline_material(self):
        ...
        p = pipeline.ensure_material(
            PipelineMaterial("deploy-qa", "baseline-user-data")
        )
        self.assertEqual(p, pipeline)
        self.assertEqual(
            PipelineMaterial("deploy-qa", "baseline-user-data"), pipeline.materials[0]
        )
    ...
    def test_materials_are_sorted(self):
        ...
        self.assertEqual(
            "git@bitbucket.org:springersbm/art.git",
            materials[0].attributes["url"].value,
        )
        self.assertEqual(
            "git@bitbucket.org:springersbm/this.git",
            materials[1].attributes["url"].value,
        )
        self.assertEqual(
            "git@bitbucket.org:springersbm/zebra.git",
            materials[2].attributes["url"].value,
        )
    ...
    def test_can_set_pipeline_git_url_for_new_pipeline(self):
        ...
        self.assertEqual(
            "git@bitbucket.org:springersbm/changed.git", new_pipeline.git_url
        )
    ...
    def test_pipelines_can_be_based_on_template(self):
        pipeline = (
            GoCdConfigurator(config("pipeline-based-on-template"))
            .ensure_pipeline_group("defaultGroup")
            .find_pipeline("siberian")
        )
    ...
    def test_pipelines_have_environment_variables(self):
        pipeline = typical_pipeline()
        self.assertEqual(
            {"JAVA_HOME": "/opt/java/jdk-1.8"}, pipeline.environment_variables
        )
    ...
    def test_pipelines_have_encrypted_environment_variables(self):
        ...
        self.assertEqual(
            {"MY_SECURE_PASSWORD": "yq5qqPrrD9/htfwTWMYqGQ=="},
            pipeline.encrypted_environment_variables,
        )

    def test_pipelines_have_unencrypted_secure_environment_variables(self):
        pipeline = (
            GoCdConfigurator(config("config-with-unencrypted-secure-variable"))
            .ensure_pipeline_group("defaultGroup")
            .find_pipeline("example")
        )
        self.assertEqual(
            {"MY_SECURE_PASSWORD": "hunter2"},
            pipeline.unencrypted_secure_environment_variables,
        )

    def test_pipelines_have_unencrypted_secure_environment_variable_unicode(self):
        pipeline = (
            GoCdConfigurator(config("config-with-unencrypted-secure-variable-unicode"))
            .ensure_pipeline_group("defaultGroup")
            .find_pipeline("example")
        )
        self.assertEqual(
            {"MY_SECURE_PASSWORD": "hunter2ª"},
            pipeline.unencrypted_secure_environment_variables,
        )
    ...
    def test_can_add_encrypted_secure_environment_variables_to_pipeline(self):
        ...
        self.assertEqual(
            {"new": "one", "again": "two"}, pipeline.encrypted_environment_variables
        )
    ...
    def test_can_add_environment_variables_to_new_pipeline(self):
        ...
        self.assertEqual(
            {"JAVA_HOME": "/opt/java/jdk-1.8", "new": "one"},
    ...
    def test_can_modify_environment_variables_of_pipeline(self):
        ...
        pipeline.ensure_environment_variables(
            {"new": "one", "JAVA_HOME": "/opt/java/jdk-1.1"}
        )
        self.assertEqual(
            {"JAVA_HOME": "/opt/java/jdk-1.1", "new": "one"},
            pipeline.environment_variables,
        )
    ...
    def test_environment_variables_get_added_in_sorted_order_to_reduce_config_thrash(
        ...
        pipeline = go_cd_configurator.ensure_pipeline_group("P.Group").ensure_pipeline(
            "P.Name"
        )
        ...
        xml = parseString(go_cd_configurator.config)
        names = [e.getAttribute("name") for e in xml.getElementsByTagName("variable")]
        self.assertEqual(["ant", "badger", "xray", "zebra"], names)

    def test_encrypted_environment_variables_get_added_in_sorted_order_to_reduce_config_thrash(
        self,
    ):
        go_cd_configurator = GoCdConfigurator(empty_config())
    ...
    def test_unencrypted_environment_variables_do_not_have_secure_attribute_in_order_to_reduce_config_thrash(
        self,
    ):
        go_cd_configurator = GoCdConfigurator(empty_config())
        ...
        xml = parseString(go_cd_configurator.config)
        secure_attributes = [
            e.getAttribute("secure") for e in xml.getElementsByTagName("variable")
        ]
        # attributes that are missing are returned as empty
        self.assertEqual(
            [""], secure_attributes, "should not have any 'secure' attributes"
        )
    ...
    def test_can_change_environment_variable_from_secure_to_insecure(self):
        ...
        self.assertEqual({"ant": "b"}, pipeline.environment_variables)
        self.assertEqual(
            {"badger": "b"}, pipeline.unencrypted_secure_environment_variables
        )
    ...
    def test_can_set_automatic_pipeline_locking(self):
        configurator = GoCdConfigurator(empty_config())
        pipeline = configurator.ensure_pipeline_group("new_group").ensure_pipeline(
            "some_name"
        )
    ...
    def test_can_have_package_repo_material(self):
        pipeline = (
            GoCdConfigurator(config("config-with-pipeline-and-yum-repo"))
            .ensure_pipeline_group("P.Group")
            .find_pipeline("test")
        )
        self.assertTrue(pipeline.has_single_package_material)
        self.assertEqual(
            pipeline.package_material.ref, "eca7f187-73c2-4f62-971a-d15233937256"
        )
    ...
    def test_can_set_pipeline_package_ref(self):
        ...
        self.assertEqual(p, pipeline)
        self.assertEqual(
            "eca7f187-73c2-4f62-971a-d15233937256", pipeline.package_material.ref
        )
...
class TestAuthorization(unittest.TestCase):
    ...
    def test_can_authorize_users_and_roles_for_operate(self):
        self.pipeline_group.ensure_authorization().ensure_operate().add_user(
            "user1"
        ).add_role("role1")

        self.assertEqual(
            self.pipeline_group.authorization.operate.users[0].username, "user1"
        )
        self.assertEqual(
            self.pipeline_group.authorization.operate.roles[0].name, "role1"
        )

    def test_can_authorize_users_and_roles_for_admins(self):
        self.pipeline_group.ensure_authorization().ensure_admins().add_user(
            "user1"
        ).add_role("role1")

        self.assertEqual(
            self.pipeline_group.authorization.admins.users[0].username, "user1"
        )
        self.assertEqual(
            self.pipeline_group.authorization.admins.roles[0].name, "role1"
        )
    ...
    def test_can_ensure_replacement_of_authorization(self):
        ...
        self.pipeline_group.ensure_replacement_of_authorization().ensure_admins().add_user(
            "user2"
        )
        self.assertEqual(len(self.pipeline_group.authorization.admins.users), 1)
...
class TestPipelineGroup(unittest.TestCase):
    def _pipeline_group_from_config(self):
        return GoCdConfigurator(
            config("config-with-two-pipelines")
        ).ensure_pipeline_group("P.Group")
    ...
    def test_can_authorize_read_only_users(self):
        pipeline_group = self._pipeline_group_from_config()
        pipeline_group.ensure_authorization().ensure_view().add_user("user1").add_user(
            "user2"
        )
    ...
    def test_does_not_find_missing_pipeline(self):
        self.assertFalse(
            self._pipeline_group_from_config().has_pipeline("unknown-pipeline")
        )
        try:
...
class TestSecurity(unittest.TestCase):
    ...
    def test_can_ensure_roles(self):
        self.configurator.ensure_security().ensure_roles().ensure_role(
            name="role_name", users=["user1", "user2"]
        )

        self.assertEqual(self.configurator.security.roles.role[0].name, "role_name")
        self.assertEqual(
            self.configurator.security.roles.role[0].users, ["user1", "user2"]
        )

    def test_can_ensure_plugin_roles(self):
        self.configurator.ensure_security().ensure_roles().ensure_plugin_role(
            name="role_name",
        ...
        self.assertEqual(
            self.configurator.security.roles.plugin_role[0].name, "role_name"
        )
        self.assertEqual(
            self.configurator.security.roles.plugin_role[0].auth_config_id,
            "id-for-auth-plugin",
        )
        self.assertEqual(
            self.configurator.security.roles.plugin_role[0].properties,
            {"SomeKey": "SomeValue"},
        )
    ...
    def test_can_ensure_replacement_of_security(self):
        ...
        self.configurator.ensure_replacement_of_security().ensure_roles().ensure_role(
            name="another_role_name", users=["user1", "user2"]
        )
        self.assertEqual(len(self.configurator.security.roles), 1)
    ...
    def test_can_ensure_replacement_of_roles(self):
        ...
        self.configurator.ensure_security().ensure_replacement_of_roles().ensure_role(
            name="another_role_name", users=["user1", "user2"]
        )
        self.assertEqual(len(self.configurator.security.roles), 1)
    ...
    def test_can_ensure_auth_config(self):
        ...
        self.configurator.ensure_security().ensure_auth_configs().ensure_auth_config(
            auth_config_id="auth-plugin-1",
            plugin_id="auth.plugin.id",
            properties=properties,
        )

        self.assertEqual(
            self.configurator.security.auth_configs[0].auth_config_id, "auth-plugin-1"
        )
        self.assertEqual(
            self.configurator.security.auth_configs[0].plugin_id, "auth.plugin.id"
        )
        self.assertEqual(
            self.configurator.security.auth_configs[0].properties, properties
        )

    def test_doesnt_add_same_plugin_twice_and_equality_is_only_by_id(self):
        self.configurator.ensure_security().ensure_auth_configs().ensure_auth_config(
            auth_config_id="auth-plugin-1", plugin_id="auth.plugin.id", properties={}
        )
        self.configurator.ensure_security().ensure_auth_configs().ensure_auth_config(
            auth_config_id="auth-plugin-1", plugin_id="another.plugin.id", properties={}
        )
    ...
    def test_can_ensure_replacement_of_auth_configs(self):
        ...
        self.configurator.ensure_security().ensure_replacement_of_auth_configs().ensure_auth_config(
            auth_config_id="auth-plugin-2", plugin_id="auth.plugin.id", properties={}
        )
    ...
    def test_can_ensure_replacement_of_auth_config(self):
        ...
        self.configurator.ensure_security().ensure_auth_configs().ensure_replacement_of_auth_config(
            auth_config_id="auth-plugin-1", plugin_id="auth.plugin.id", properties={}
        )
...
class TestElastic(unittest.TestCase):
    ...
    def test_can_ensure_elastic(self):
        properties = {"key": "value"}
        self.configurator.ensure_elastic().ensure_profiles().ensure_profile(
            profile_id="unit-test",
            plugin_id="cd.go.contrib.elastic-agent.docker",
            properties=properties,
        )
        ...
        self.assertEqual(
            self.configurator.elastic.profiles[0].plugin_id,
    ...
    def test_can_ensure_replacement_of_elastic(self):
        ...
        self.configurator.ensure_replacement_of_elastic().ensure_profiles().ensure_profile(
            profile_id="unit-test",
            plugin_id="cd.go.contrib.elastic-agent.docker",
            properties={},
        )
    ...
    def test_can_ensure_replacement_of_profiles(self):
        ...
        self.configurator.ensure_elastic().ensure_replacement_of_profiles().ensure_profile(
    ...
    def test_can_ensure_replacement_of_profile(self):
        ...
        self.configurator.ensure_elastic().ensure_profiles().ensure_replacement_of_profile(
            profile_id="unit-test",
            plugin_id="cd.go.contrib.elastic-agent.docker",
            properties={},
        )
...
class TestGoCdConfigurator(unittest.TestCase):
    def test_can_tell_if_there_is_no_change_to_save(self):
        ...
        p = configurator.ensure_pipeline_group(
            "Second.Group"
        ).ensure_replacement_of_pipeline("smoke-tests")
        p.set_git_url("git@bitbucket.org:springersbm/gomatic.git")
        p.ensure_stage("build").ensure_job("compile").ensure_task(
            ExecTask(["make", "source code"])
        )
    ...
    def test_keeps_schema_version(self):
        empty_config = FakeHostRestClient(
            empty_config_xml.replace('schemaVersion="72"', 'schemaVersion="73"'),
            "empty_config()",
        )
        configurator = GoCdConfigurator(empty_config)
    ...
    def test_gets_all_pipeline_groups(self):
        self.assertEqual(
            ...
            len(
                GoCdConfigurator(
                    config("config-with-two-pipeline-groups")
                ).pipeline_groups
            ),
        )
    ...
    def test_config_is_updated_as_result_of_updating_part_of_it(self):
        ...
        self.assertEqual(2, len(agent.resources))
        agent.ensure_resource("a-resource-that-it-does-not-already-have")
        configurator_based_on_new_config = GoCdConfigurator(
            FakeHostRestClient(configurator.config)
        )
    ...
    def test_can_ensure_removal_of_pipeline_group(self):
        configurator = GoCdConfigurator(config("config-with-two-pipeline-groups"))
        configurator.ensure_removal_of_pipeline_group(
            "pipeline-group-that-has-already-been-removed"
        )
        self.assertEqual(2, len(configurator.pipeline_groups))
    ...
    def test_can_add_template(self):
        ...
        self.assertTrue(
            isinstance(configurator.templates[0], Pipeline),
            "so all methods that use to configure pipeline don't need to be tested for template",
        )
    ...
    def test_top_level_elements_get_reordered_to_please_go(self):
        configurator = GoCdConfigurator(
            config("config-with-agents-and-templates-but-without-pipelines")
        )
        configurator.ensure_pipeline_group("some_group").ensure_pipeline(
            "some_pipeline"
        )
        xml = configurator.config
        root = ET.fromstring(xml)
    ...
    def test_top_level_elements_with_environment_get_reordered_to_please_go(self):
        configurator = GoCdConfigurator(
            config("config-with-pipelines-environments-and-agents")
        )
    ...
    def test_top_level_elements_that_cannot_be_created_get_reordered_to_please_go(self):
        ...
        root = ET.fromstring(xml)
        self.assertEqual(
            ["server", "repositories", "scms", "pipelines", "environments", "agents"],
            [element.tag for element in root],
        )

    def test_elements_can_be_created_in_order_to_please_go(self):
        configurator = GoCdConfigurator(empty_config())
        pipeline = configurator.ensure_pipeline_group("some_group").ensure_pipeline(
            "some_pipeline"
        )
...
class TestRepository(unittest.TestCase):
    def test_can_read_yum_repo_from_xml(self):
        ...
        self.assertEqual(
            yum_repository.repo_url, "http://yum-server/releases/component-name/"
        )
    ...
    def test_can_ensure_type_on_populated_repository(self):
        ...
        prop = yum_repository.ensure_property(
            "REPO_URL", "http://yum-server/releases/component-name/"
        )
...
class TestReverseEngineering(unittest.TestCase):
    def check_round_trip_pipeline(self, configurator, before, show=False):
        ...
        if before.is_based_on_template:
            # noinspection PyTypeChecker
            self.assertEqual(
                sneakily_converted_to_xml(before.template),
                sneakily_converted_to_xml(template),
            )
    ...
    def test_can_round_trip_standard_label(self):
        configurator = GoCdConfigurator(empty_config())
        before = (
            configurator.ensure_pipeline_group("group")
    ...
    def test_can_round_trip_non_standard_label(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .set_label_template("non standard")
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_lock_behavior(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .set_lock_behavior("lockOnFailure")
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_pipeline_material(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .ensure_material(PipelineMaterial("p", "s", "m"))
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_git_branch_only(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .set_git_material(GitMaterial("some git url", branch="some branch"))
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_git_material_only(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .set_git_material(GitMaterial("some git url", material_name="m name"))
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_git_polling_only(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .set_git_material(GitMaterial("some git url", polling=False))
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_git_ignore_patterns_only_ISSUE_4(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .set_git_material(GitMaterial("git url", ignore_patterns={"ex", "cluded"}))
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_git_destination_directory_only(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .set_git_material(GitMaterial("git url", destination_directory="foo/bar"))
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_pipeline_environment_variables(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .ensure_environment_variables({"p": "v"})
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_pipeline_encrypted_environment_variables(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .ensure_encrypted_environment_variables({"p": "v"})
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_pipeline_unencrypted_secure_environment_variables(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .ensure_unencrypted_secure_environment_variables({"p": "v"})
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_timer_only_on_changes(self):
        ...
        before = (
            configurator.ensure_pipeline_group("group")
            .ensure_pipeline("line")
            .set_timer("some timer", only_on_changes=True)
        )
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_stage_bits(self):
        configurator = GoCdConfigurator(empty_config())
        before = configurator.ensure_pipeline_group("group").ensure_pipeline("line")
        before.ensure_stage("stage1").ensure_environment_variables(
            {"k": "v"}
        ).set_clean_working_dir().set_has_manual_approval().set_fetch_materials(False)
        self.check_round_trip_pipeline(configurator, before)
    ...
    def test_can_round_trip_job_bits(self):
        configurator = GoCdConfigurator(empty_config())
        before = configurator.ensure_pipeline_group("group").ensure_pipeline("line")
        before.ensure_stage("stage").ensure_job("job").ensure_artifacts(
            {Artifact.get_build_artifact("s", "d"), Artifact.get_test_artifact("sauce")}
        ).ensure_environment_variables({"k": "v"}).ensure_resource("r").ensure_tab(
            Tab("n", "p")
        ).set_timeout(
            "23"
    ...
    def test_can_round_trip_tasks(self):
        ...
        job.ensure_task(
            FetchArtifactTask("p", "s", "j", FetchArtifactFile("f"), runif="any")
        )
        job.ensure_task(FetchArtifactTask("p", "s", "j", FetchArtifactDir("d")))
        job.ensure_task(
            FetchArtifactTask(
                "p", "s", "j", FetchArtifactDir("d"), dest="somewhere-else"
            )
        )
        job.ensure_task(
            FetchArtifactTask(
                "p", "s", "j", FetchArtifactDir("d"), dest="somewhere-else", runif="any"
            )
        )
...
class TestXmlFormatting(unittest.TestCase):
    ...
    def test_can_format_actual_config(self):
        ...
        self.assertEqual(
            ...
            formatted,
            "expected=\n%s\n%s\nactual=\n%s"
            % (head(expected), "=" * 88, head(formatted)),
        )
...
class TestArtifacts(unittest.TestCase):
    ...
    def test_can_go_from_xml_to_build_artifact_object_with_version_gocd_18_2_and_below(
        self,
    ):
    ...
    def test_can_go_from_xml_to_test_artifact_object_with_version_gocd_18_2_and_below(
        self,
    ):
    ...
    def test_can_go_from_xml_to_build_artifact_object_with_version_gocd_18_3_and_above(
        self,
    ):
        element = ET.Element("artifact")
        element.attrib["src"] = "src"
    ...
    def test_can_go_from_xml_to_test_artifact_object_with_version_gocd_18_3_and_above(
        self,
    ):
        element = ET.Element("artifact")
        element.attrib["src"] = "src"
...
class TestArtifactStores(unittest.TestCase):
    ...
    def test_can_ensure_artifact_store(self):
        properties = {"key1": "value", "key2": None}
        self.configurator.ensure_artifact_stores().ensure_artifact_store(
            "id1", "plugin_id1", properties
        )
        self.configurator.ensure_artifact_stores().ensure_artifact_store(
            "id2", "plugin_id2", properties
        )
    ...
    def test_can_ensure_replacement_of_artifact_stores(self):
        ...
        self.configurator.ensure_replacement_of_artifact_stores().ensure_artifact_store(
            "id2", "plugin_id2", properties
        )
        self.assertEqual(len(self.configurator.artifact_stores), 1)
    ...
    def test_can_ensure_replacement_of_artifact_store(self):
        ...
        self.configurator.ensure_artifact_stores().ensure_artifact_store(
            "id", "plugin_id", properties1
        )
        self.assertEqual(len(self.configurator.artifact_stores), 1)
        self.assertEqual(self.configurator.artifact_stores[0].properties, properties1)
        self.configurator.ensure_artifact_stores().ensure_replacement_of_artifact_store(
            "id", "plugin_id", properties2
        )
        self.assertEqual(len(self.configurator.artifact_stores), 1)
    ...
    def test_can_differentiate_artifact_store(self):
        ...
        self.assertNotEqual(
            self.configurator.artifact_stores[0], self.configurator.artifact_stores[1]
        )
