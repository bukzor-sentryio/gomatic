...
def start_go_server(gocd_version, gocd_download_version_string, gocd_port):
    os.system(
        "./build-and-run-go-server-in-docker.sh %s %s %s"
        % (gocd_version, gocd_download_version_string, gocd_port)
    )
...
class populated_go_server:
    ...
    def __enter__(self):
        try:
            start_go_server(
                self.gocd_version, self.gocd_download_version_string, self.gocd_port
            )
...
def check_installed(
    command, desired_version, versionNumberFromVersionOutput, installationInstructions
):
    try:
        installed_version = versionNumberFromVersionOutput(
            subprocess.check_output([command, "--version"])
        )
        if StrictVersion(desired_version) > StrictVersion(installed_version):
            fail_with(
                "Need %s version %s+. You have %s."
                % (command, desired_version, installed_version)
            )
    except OSError:
...
class IntegrationTest(unittest.TestCase):
    gocd_versions = [
        ...
        ("18.6.0-9515", "_19.6.0-9515_all"),
        ("18.7.0-9515", "_19.6.0-9515_all"),
        ("18.8.0-7433", "_18.8.0-7433_all"),
        ("18.9.0-7478", "_18.9.0-7478_all"),
        ("18.10.0-7703", "_18.10.0-7703_all"),
        ("18.11.0-8024", "_18.11.0-8024_all"),
        ("18.12.0-8222", "_18.12.0-8222_all"),
        ("19.1.0-8469", "_19.1.0-8469_all"),
        ("19.2.0-8641", "_19.2.0-8641_all"),
        ("19.3.0-8959", "_19.3.0-8959_all"),
        ("19.4.0-9155", "_19.4.0-9155_all"),
        ("19.5.0-9272", "_19.5.0-9272_all"),
        ("19.6.0-9515", "_19.6.0-9515_all"),
    ]
    ...
    def ignore_test_can_save_multiple_times_using_same_configurator(self):
        ...
        print(
            "test_can_save_multiple_times_using_same_configurator",
            "*" * 60,
            gocd_version,
        )
        with populated_go_server(
            ...
            pipeline.set_git_material(
                GitMaterial("https://github.com/SpringerSBM/gomatic.git", polling=False)
            )
...
if __name__ == "__main__":
    if not os.path.exists("go-server-%s.deb" % IntegrationTest.gocd_versions[0][0]):
        print(
            "This takes a long time to run first time, because it downloads a Java docker image and GoCD .deb packages from the internet"
        )
        check_docker()
