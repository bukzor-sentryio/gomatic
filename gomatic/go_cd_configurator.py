...
class GoCdConfigurator:
    ...
    def __set_initial_config_xml(self):
        ...
        if isinstance(initial_config, bytes):
            self.__initial_config = initial_config.decode(
                "ascii", errors="xmlcharrefreplace"
            )
        else:
            self.__initial_config = initial_config.encode(
                "ascii", errors="xmlcharrefreplace"
            )
    ...
    def __set_server_version(self):
        ...
        if response.status_code == 404:
            self.__server_version = (
                "16.5.0"  # hardcoding to version before endpoint created
            )
            ...
            raise Exception(
                "Failed to get {} status {}\n:{}".format(
                    version_url, response.status_code, response.text
                )
            )
    ...
    def __current_config_response(self):
        ...
        if response.status_code != 200:
            raise Exception(
                "Failed to get {} status {}\n:{}".format(
                    config_url, response.status_code, response.text
                )
            )
    ...
    def agent_auto_register_key(self):
        return self.__possibly_missing_server_element().attribute(
            "agentAutoRegisterKey"
        )

    @agent_auto_register_key.setter
    def agent_auto_register_key(self, agent_auto_register_key):
        self.__server_element_ensurance().set(
            "agentAutoRegisterKey", agent_auto_register_key
        )
    ...
    def ensure_pipeline_group(self, group_name):
        pipeline_group_element = Ensurance(self.__xml_root).ensure_child_with_attribute(
    ...
    def repositories(self):
        return [
            ...
            .possibly_missing_child("repositories")
            .findall("repository")
        ]

    def ensure_repository(self, repository_name):
        ensured_repository_element = (
            Ensurance(self.__xml_root)
            .ensure_child("repositories")
            .ensure_child_with_attribute("repository", "name", repository_name)
        )
    ...
    def templates(self):
        return [
            ...
            for e in PossiblyMissingElement(self.__xml_root)
            .possibly_missing_child("templates")
            .findall("pipeline")
        ]
    ...
    def ensure_security(self):
        security_element = (
            self.__server_element_ensurance().ensure_child("security").element
        )
    ...
    def ensure_elastic(self):
        ...
        if gocd_major_version < gocd_version_with_elastic_outside_server_tag:
            elastic_element = (
                self.__server_element_ensurance().ensure_child("elastic").element
            )
        else:
    ...
    def elastic(self):
        ...
        gocd_major_version = int(self.server_version.split(".")[0])
        if gocd_major_version < gocd_version_with_elastic_outside_server_tag:
            elastic_element = Elastic(
                self.__server_element_ensurance().element.find("elastic")
            )
        else:
            elastic_element = Elastic(
                Ensurance(self.__xml_root).element.find("elastic")
            )
    ...
    def save_updated_config(self, save_config_locally=False, dry_run=False):
        ...
        data = {"xmlFile": self.config, "md5": self._initial_md5}
        ...
        if not dry_run and config_before != config_after:
            ...
            if self.__host_rest_client.access_token is not None:
                headers["Authorization"] = (
                    "Bearer %s" % self.__host_rest_client.access_token
                )
...
class HostRestClient:
    ...
    def get(self, path):
        ...
        count = 0
        while ((result.status_code == 503) or (result.status_code == 504)) and (
            count < 5
        ):
    ...
    def post(self, path, data, headers=None):
        ...
        result = requests.post(
            url, data, auth=self.__auth(), verify=self.__verify_ssl, headers=headers
        )
        if result.status_code != 200:
            try:
                ...
                raise RuntimeError(
                    "Could not post config to Go server (%s) [status code=%s]:\n%s"
...
def main(args):
    parser = argparse.ArgumentParser(
        description="Gomatic is an API for configuring GoCD. "
        "Run python -m gomatic.go_cd_configurator to reverse engineer code to configure an existing pipeline."
    )
    parser.add_argument(
        "-s", "--server", help='the go server (e.g. "localhost:8153" or "my.gocd.com")'
    )
    parser.add_argument(
        "-p",
        "--pipeline",
        help="the name of the pipeline to reverse-engineer the config for",
    )
    parser.add_argument(
        "--username", help="the username for the gocd server", default=None
    )
    parser.add_argument(
        "--password", help="the password for the gocd server", default=None
    )
    parser.add_argument(
        "--ssl",
        help="use HTTPS for the connection to the gocd server",
        dest="ssl",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--verify_ssl",
        help="if set the identity of the ssl certificate will be verified.",
        dest="verify_ssl",
        action="store_true",
        default=False,
    )
    ...
    if len(matching_pipelines) != 1:
        raise RuntimeError(
            "Should have found one matching pipeline but found %s" % matching_pipelines
        )
