...
class Repository(CommonEqualityMixin):
    def __init__(self, element):
        ...
        self.__properties = list(
            map((lambda e: Property(e)), element.findall("configuration/property"))
        )
        self.__packages = list(
            map((lambda e: Package(e)), element.findall("packages/package"))
        )
    ...
    def ensure_type(self, type, version):
        plugin_configuration = Ensurance(self.__element).ensure_child_with_attribute(
    ...
    def ensure_property(self, key, value):
        ...
        property_element = (
            Ensurance(config_element)
            .ensure_child_with_descendant("property", "key", key)
            .element
        )
    ...
    def ensure_package(self, name):
        ens = (
            Ensurance(self.__element)
            .ensure_child("packages")
            .ensure_child_with_attribute("package", "name", name)
        )
