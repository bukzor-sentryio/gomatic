...
class ResourceMixin:
    ...
    def ensure_resource(self, resource):
        if resource not in self.resources:
            Ensurance(self.element).ensure_child("resources").append(
                ET.fromstring("<resource>%s</resource>" % resource)
            )
        return self
...
class EnvironmentVariableMixin:
    ...
    def __is_secure(variable_element):
        return (
            "secure" in variable_element.attrib
            and variable_element.attrib["secure"] == "true"
        )
    ...
    def __environment_variables(self, secure, encrypted=False):
        guarded_element = PossiblyMissingElement(self.element)
        variable_elements = guarded_element.possibly_missing_child(
        ...
        for variable_element in variable_elements:
            if secure == self.__is_secure(variable_element):
                ...
                if encrypted == is_encrypted:
                    result[variable_element.attrib["name"]] = variable_element.find(
                        value_element_name
                    ).text
        return result

    def __ensure_environment_variables(self, environment_variables, secure, encrypted):
        ensured_env_variables = Ensurance(self.element).ensure_child(
            "environmentvariables"
        )
        for env_variable in sorted(environment_variables.keys()):
            variable_element = ensured_env_variables.ensure_child_with_attribute(
                "variable", "name", env_variable
            )
            if secure:
                variable_element.set("secure", "true")
            else:
                PossiblyMissingElement(variable_element.element).remove_attribute(
                    "secure"
                )
            value_element = variable_element.ensure_child(
                self.__value_element_name(encrypted)
            )
    ...
    def ensure_environment_variables(self, environment_variables):
        self.__ensure_environment_variables(
            environment_variables, secure=False, encrypted=False
        )
        return self

    def ensure_encrypted_environment_variables(self, environment_variables):
        self.__ensure_environment_variables(
            environment_variables, secure=True, encrypted=True
        )
        return self

    def ensure_unencrypted_secure_environment_variables(self, environment_variables):
        self.__ensure_environment_variables(
            environment_variables, secure=True, encrypted=False
        )
        return self
    ...
    def as_python(self):
        ...
        if self.encrypted_environment_variables:
            result += (
                ".ensure_encrypted_environment_variables(%s)"
                % self.encrypted_environment_variables
            )

        if self.unencrypted_secure_environment_variables:
            result += (
                ".ensure_unencrypted_secure_environment_variables(%s)"
                % self.unencrypted_secure_environment_variables
            )
