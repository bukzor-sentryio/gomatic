...
class ConfigRepos(CommonEqualityMixin):
    ...
    def config_repo(self):
        return [
            ConfigRepo(e, self.__configurator.server_version)
            for e in self.element.findall("config-repo")
        ]

    def make_empty(self):
        PossiblyMissingElement(self.element).remove_all_children()

    def ensure_config_repo(
        self, url, plugin, cvs="git", configuration=None, repo_id=None, branch=None
    ):
        ...
        if has_id(self.__configurator.server_version):
            ...
            attr_name = (
                "pluginId"
                if has_new_attr_name(self.__configurator.server_version)
                else "plugin"
            )
            element = ET.fromstring(
                '<config-repo {5}="{0}" id="{4}"><{2} url="{1}"{6} />{3}</config-repo>'.format(
                    plugin,
                    url,
                    cvs,
                    configuration_xml_string,
            ...
            element = ET.fromstring(
                '<config-repo plugin="{0}"><{2} url="{1}"{4} />{3}</config-repo>'.format(
    ...
    def ensure_replacement_of_config_repo(
        self, url, plugin, cvs="git", configuration=None
    ):
    ...
    def ensure_yaml_config_repo(self, git_url):
        return self.ensure_config_repo(
            git_url, "yaml.config.plugin", cvs="git", configuration=None
        )

    def ensure_json_config_repo(self, git_url):
        return self.ensure_config_repo(
            git_url, "json.config.plugin", cvs="git", configuration=None
        )
