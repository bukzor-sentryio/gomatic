...
class AuthConfigs(CommonEqualityMixin):
    ...
    def ensure_auth_config(self, auth_config_id, plugin_id, properties):
        ...
        auth_config = ET.fromstring(
            '<authConfig id="{}" pluginId="{}">{}</authConfig>'.format(
                auth_config_id, plugin_id, properties_xml
            )
        )
    ...
    def ensure_replacement_of_auth_config(self, auth_config_id, plugin_id, properties):
        current_auth_config = [
            ac for ac in self.auth_config if ac.auth_config_id == auth_config_id
        ]
