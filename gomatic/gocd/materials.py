...
def Materials(element):
    ...
    if element.tag == "pipeline":
        ...
        return PipelineMaterial(
            element.attrib["pipelineName"], element.attrib["stageName"], material_name
        )
    if element.tag == "package":
        return PackageMaterial(element.attrib.get("ref", None))
    raise RuntimeError(
        "don't know of material matching " + ET.tostring(element, "utf-8")
    )
...
class GitMaterial(CommonEqualityMixin):
    ...
    def __repr__(self):
        ...
        if self.destination_directory:
            destination_directory_part = (
                ', destination_directory="%s"' % self.destination_directory
            )
        invert_filter_part = ""
        ...
        if self.ignore_patterns:
            ...
            for ignore_pattern in sorted_ignore_patterns:
                filter_element.append(
                    ET.fromstring('<ignore pattern="%s"/>' % ignore_pattern)
                )
...
class PipelineMaterial(CommonEqualityMixin):
    ...
    def append_to(self, element):
        if self.__material_name is None:
            new_element = ET.fromstring(
                '<pipeline pipelineName="%s" stageName="%s" />'
